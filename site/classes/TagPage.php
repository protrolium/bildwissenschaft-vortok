...
class TagPage extends Page {
    public function init() {
        // test to see if init method is working in the Tracy Debugger Dump … making sure this is in ready.php: $pages->get("template=tag")->init();
        bd("init method");

        // Call wpCategory() when Page is saved to get the WP API data
    ...
    public function wpCategory() {
        ...
        $categoryResult = $http->get('https://bildwissenschaft.vortok.info/wp-json/wp/v2/categories/?per_page=60');
        ...
        foreach ($categoriesMap as $wpId => $title) {
            ...
            $existingPage = $pages->get("categories=$wpId, template=tag");
            if ($existingPage->id) {
