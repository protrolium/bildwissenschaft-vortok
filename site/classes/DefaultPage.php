...
class DefaultPage extends Page {
    public function init() {
        // test to see if init method is working in the Tracy Debugger Dump … making sure this is in ready.php: $pages->get("template=default-page")->init();				
        bd("init method");
    ...
    public function wpPost() {
        ...
        // posts endpoint
        $result = $http->get('https://bildwissenschaft.vortok.info/wp-json/wp/v2/posts/?per_page=70');
        ...
        foreach ($optimizedArray as $post) {
            ...
            $postsResult = $http->get('https://bildwissenschaft.vortok.info/wp-json/wp/v2/posts/' . $post->id);
            $postsResult = json_decode($postsResult);

            // media endpoint
            $featuredMediId = $postsResult->featured_media;
            $featuredImageResult = $http->get('https://bildwissenschaft.vortok.info/wp-json/wp/v2/media/' . $featuredMediId);
